...
def save_per_epoch_logs(output_dir, model_name, run_number, seed, training_stats):
    """
    Save per-epoch validation loss for creating validation loss evolution plots
    This creates a separate file with epoch-by-epoch data for all runs
    """
    # Path for per-epoch logs
    epoch_log_file = Path(output_dir) / f"{model_name}_per_epoch_logs.csv"
    ...
    for stat in training_stats:
        epoch_rows.append({
            ...
            "seed": seed,
            "epoch": stat["epoch"],
            "train_loss": stat["Training Loss"],
            "val_loss": stat["Valid. Loss"],
            "f1_score": stat["F1 score"],
        })
    
    # Append to CSV
    new_df = pd.DataFrame(epoch_rows)
    if epoch_log_file.exists():
        existing_df = pd.read_csv(epoch_log_file)
        combined_df = pd.concat([existing_df, new_df], ignore_index=True)
    else:
        combined_df = new_df
    
    combined_df.to_csv(epoch_log_file, index=False)
    print(f"Per-epoch logs appended to: {epoch_log_file}")
    
    return epoch_log_file
...
for run_idx in range(fine_tuning_runs):
    ...
    # NEW: Save per-epoch logs for validation loss evolution plot
    save_per_epoch_logs(
        output_dir=args.output_dir,
        model_name=model_name,
        run_number=args.run_number,
